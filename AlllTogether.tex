...
% if you use draft as option the figures don't show up.
\usepackage{amsmath}
\usepackage{graphicx}
...
\begin{document}
\title{Search for 140 $\mu$eV Axion-Like Particle Dark Matter Using Microwave Cavities}
\author{Ana T. Malagon}
\advisorline{Dissertation Advisor: O. Keith Baker}
\date{}
...
We now discuss the motivation for axions (and ALPs) in more detail. 
...
CP symmetry, or charge parity symmetry, refers to the symmetry of physical equations under reversal of spatial reflection and charge conjugation.  When CP violation occurs,  it manifests as a visible distinction between particles and antiparticles \cite{gottfried86}. The electroweak interaction violates CP symmetry, as observed in neutral kaon systems \cite{christenson64}, however there is no evidence for strong interaction processes which violate CP. This is odd, as there is a term in the QCD Lagrangian that is explicitly P- and T- violating:
\begin{equation}\label{eq:cpterm}
\mathcal{L} = \bar \theta \frac{\alpha_s}{4\pi} G \tilde G
\end{equation}
where $\bar \theta$ is a phase between $0$ and $2\pi$, $G$ is the gluon field tensor and $\tilde G$ is the dual, or $G_{ab} = \epsilon_{abcd} G^{cd}$. To see that it is parity and time violating, construct the color electric and magnetic field equivalents from the tensor. Recall that for the electromagnetic field tensor $FF = E^2 -B^2$ and $F\tilde F = E\cdot B$. Therefore for the gluon field tensor, $G\tilde G = E_c \cdot B_c$, for the equivalent color electric and magnetic fields. . Under a parity transformation ($x \rightarrow -x$), $E \rightarrow E$ and $B \rightarrow -B$. Under a time transformation, $
E \rightarrow E$ and $B \rightarrow -B$.  Therefore the product $E\cdot B$ is not symmetric under parity and time reversal. By the CPT theorem, T-violation is equivalent to CP-violation.

We note that the reason the equivalent CP violating term  ($F\tilde F$) does not appear in the Lagrangian for QED is because it is a total four-divergence. This means, for fields that are vanishingly small as you approach spatial infinity, that the term goes to zero when integrating the Lagrangian. Because the gluon fields are self-interacting the term $G \tilde G$ is not zero as you go to infinity, and so the four divergence has a finite effect.

This CP violating term should induce observable effects, such as a permanent neutron electric dipole moment of order
\begin{align}
|d_n| \sim \frac{e}{m_n}\big(\frac{m_u}{m_n}\big)|\bar \theta| \sim 10^{-16} |\bar \theta | \text{ e}\cdot\text{cm}
\end{align}
An observation of a permanent electric dipole moment for any particle would be a major achievement, as no particle has yet been observed to have such a permanent electric dipole moment, although all fermions have magnetic dipole moments. However measurements of the neutron EDM $d_n$ place upper bounds  at the level $d_n < 2.9 \times10^{-26}$ e$\cdot$cm \cite{baker06}, placing an upper bound on the QCD phase of $\bar \theta < 10^{-10}$ . The strong CP problem is the question of why $\bar \theta$ is so small, when a ``natural" value for this periodic variable would be of order unity.

The P, T violating phase $\bar \theta$ can be set to zero, however this constitutes another fine tuning problem as $\bar \theta$ is the sum of two independent terms from two different sectors, which should be independent:
\begin{align}
\bar \theta = \theta_{QCD} + \text{arg det} \mathcal{M}
\end{align}
$\theta_{QCD}$ is a phase that comes from strong interaction physics; it can be thought of as the term characterizing the ground state of the QCD vacuum, which is a superposition of various vacua connected by tunneling events called instantons.

%Any non-Abelian field may have such a non-trivial vacuum structure; the evidence for  QCD having a non-trivial vacua comes from of such a vacua solves the $U(1)_A$ problem, which is the question of why the $\eta'$ is not degenerate with the pion mass \cite{thooft76}. The problem is a question of symmetry breaking - [NEEDSEXPLANATION]. 
...
$\mathcal{M}$ is the quark mass matrix: $\text{arg det } \mathcal{M} = m_um_dm_s$ the product of the quark masses when real. If there is a massless quark, the value above is undefined and $\bar \theta$ is also undefined. However, we believe there is evidence that the lightest quark, the up, is not massless and has mass between 0.3-0.7 MeV [NEEDSREF].
 ...
 The strong-CP problem can be solved by introducing a new global, chiral symmetry, as proposed by Peccei and Quinn in 1977 \cite{peccei77}. The spontaneous breaking of this symmetry at some energy scale $f_a$ introduces the axion as the resulting massless Goldstone boson of the symmetry. Due to a chiral anomaly with QCD, the axion experiences a potential, which causes it to acquire a mass that depends on  the energy scale of QCD, $\Lambda_{QCD}$, and is inversely proportional to the energy scale of the symmetry breaking $f_a$ \cite{weinberg78}, \cite{wilczek78}. 
...
The Peccei-Quinn solution to the strong CP problem introduces  a global chiral $U(1)$ symmetry, denoted $U(1)_{PQ}$. This symmetry is spontaneously broken at some energy scale $f_a$, producing massless Goldstone bosons from the azimuthal degree of freedom (this is the axion), and some high energy mode from the radial degree of freedom; this was deemed to be the two Higgs doublets in the original theory, which sets the energy scale.  There are many models that construct other scalars as the radial mode, allowing the energy scale to decouple from the electroweak scale \cite{kim79} \cite{shifman80}, \cite{dine81}, \cite{zhitnisky80}.

Due to a chiral anomaly of QCD with this new symmetry \cite{jackiw76} a potential arises for the axion, which allows the axion to acquire mass. Neglecting temperature effects, this potential can be parameterized as 
\begin{align}
V(a) = \frac{a^2}{f_a^2}(1 - \cos^2 (\frac{a}{f_a}))
\end{align}
with the corresponding term in the Lagrangian as
\begin{align}
\mathcal{L}_a = \frac{a}{f_a}G\tilde G
\end{align}
Note that the axion Lagrangian term has the same form as the original CP violating term (Eq~\ref{eq:cpterm}). Minimizing the potential for the axion causes the axion expectation value to settle at $f_a \bar \theta$, which exactly cancels the original CP violating term in the QCD Lagrangian. Thus all CP violating effects go to zero in a natural way, with an observable consequence being the new pseudo-Goldstone boson of the symmetry, the axion. 

This exact process occurs in QCD for another particle, the pion. As the Goldstone boson of the axial $U(1)_A$ symmetry in QCD, the pion acquires its mass through a chiral anomaly as well. The axion mass is proportional to the QCD scale: $m_a \sim \Lambda_{QCD}^2/f_a$. The axion coupling to matter is inversely proportional to the energy scale $f_a$ and thus proportional to the mass.

Using Higgs doublets to parametrize the potential would give one a rough prediction for the axion mass of $m_a \sim 150$ keV; this axion would have a lifetime of 1 microsecond and would have been detected in collider experiments through quarkonium decays and beam dump experiments \cite{crystalball90}. The branching ratios observed were significantly less than predicted; thus this axion was ruled out.

However, as stated above, the symmetry can come from other scalars which would separate $f_a$ from the electroweak scale and let it be much higher, and so the axion coupling and mass would be much weaker. There are two benchmark models, the KSVZ and DFSZ axion models, that use different mechanisms to give the axion mass, but in the end both predict similar axion to two photon couplings that differ by at most a factor of 3.
...
In conclusion, for the two photon interaction the coupling is
\begin{align}
g_{a\gamma\gamma} &= -\frac{3\alpha}{8\pi f_a}\xi = - \frac{m_a/\text{eV}}{0.69\times10^{10}\text{ GeV}}\xi 
\\ \xi &= \frac{4}{3}(\frac{E}{N} - 1.92 \pm 0.08)
\end{align}

$E/N$ is the ratio of the color to electromagnetic anomalies. For the DFSZ mode it is 8/3, for the KSVZ model it is 0. It cannot be guaranteed that $E/N$ does not equal 2, however, in which case the coupling to photons would be severely suppressed.

 In 1983 Pierre Sikivie proposed that axions with extremely feeble couplings would make good dark matter candidates  \cite{sikivie83}, and outlined a technique to search for axions using the two photon interaction. This interaction can be written as $E \cdot B$. Using strong laboratory magnetic fields and sensitive photon detectors, experimentalists can detect dark matter axions converting to photons.
...
However, in order for ALPs to be good dark matter candidates, they must have the correct abundance. We now go over how ALPs could be produced in the early universe such as to have the correct density today.
%
...
%
%[NEEDSALOTOFWORK]
%[MAKENOTATIONCONSISTENT]
The misalignment mechanism refers to a non-thermal process of producing axions in the early universe. The present day abundance of these primordial axions will be dictated by the initial value of the angle, $\bar \theta = \phi_0$, and the evolution of the axion field $\phi$. 

The evolution of the axion field is dictated by the equations of motion in the Robertson-Walker metric. As spatial variations are smoothed by the expansion of the universe, we take the field to be spatially homogeneous ($\partial_i \phi = 0$). Then the equations of motion for the zero momentum component of the field are
\begin{equation}\label{eq:harmonic}
\ddot \phi + 3 H(T) \dot \phi + V'(\phi, T) = 0
\end{equation}
where $V(\phi, T)$ is the axion potential, $\phi$ the axion field, and $H(T)$ the Hubble parameter. For simplicity of notation we drop the temperature dependence of all terms in the following discussion.
Eq.~\ref{eq:harmonic} shows that the field acts like a damped harmonic oscillator with damping term given by the Hubble parameter. For $ H > m_\phi$ the field will be over damped and $\phi = \phi_0$ will stay at its initial value. In the early universe, the axion field is massless and so there is no preferred value for the initial term: $\phi_0$ is some random value between 0 and $2\pi$. The total energy density in the field is the potential energy $\frac{1}{2} m_\phi \phi_0^2$. When the temperature decreases after inflation, the Hubble parameter will become less than $m_\phi$; this happens when $T \approx (m_\phi M_{pl})^2$. After this point the field begins to oscillate in the potential and the total energy will be a combination of potential and kinetic energy. The quantized oscillations of the field are the particles: axions.The kinetic energy will redshift like matter as 
\begin{align}
\rho_\phi \propto a^{-3}
\end{align}
with $a$ the scale factor.

Therefore 
\begin{align}
\rho_\phi(a) = \frac{1}{2}m_\phi \phi_0^2 \big(\frac{a_0}{a}\big)^3
\end{align}

which leads to a density parameter today of 
\begin{align}
\Omega_{0, \phi} \approx \big(\frac{\phi_0^4 m_\phi}{10^{-19}\text{ GeV}^5}\big)^{1/2}
\end{align}

For QCD axions with $\phi_0 = \langle \phi \rangle \approx f_a$ and $m_a \approx \Lambda_{QCD}^2 / f_a$, with $\Lambda_{QCD} \approx 0.3$ GeV, 
the density parameter is
\begin{equation}\label{eq:density}
\Omega_{0, \phi} \approx \big(\frac{f_a}{10^{13}\text{ GeV}}\big)^{1/2}
\end{equation}
This energy density represents a coherent state of zero-momentum particles.
...
The preceding discussion assumed that symmetry breaking happened before inflation. If, however, inflation occurs during or after the symmetry breaking, the scalar field $\sigma = f_a e^{i\phi/f_a}$ will acquire random angles in causally unconnected regions of space-time. Then the present universe originated from many regions with initially uncorrelated $\langle \phi \rangle$, and to estimate the present day abundance, we must average over all possible $\phi$. In this scenario, axion production from strings and other topological defects become significant.

\subsection{Axion Strings}
Since $\sigma$ is single-valued, the total change in phase around any closed path must be $\Delta \theta = 2 \pi n$, where n is an integer, called the winding number. For  $n >0$, it is not possible to shrink the loop radius to zero without encountering a singularity where the Higgs field vanishes. The resulting tubes of false vacuum are called strings.
...
Other topological defects such as domain walls can also produce axions. There are large uncertainties in estimates of the axion mass and coupling from these production mechanisms, so we do not go into further detail here. For a review, see \cite{vilenkin00}.
...
For a light massive vector in a FRW universe, the equation of motion is $\partial_\mu (\phi^{\mu\nu}\sqrt{-g}) = M^2 \phi^\mu \sqrt{-g}$. As inflation blows up a small patch of space, the dark photon is uniformly distributed and picks a particular polarization. Then in the cosmic frame $\partial_i \phi^\mu = 0$, and from the previous equation $\phi_0 = 0$ as long as M is not 0. The spatial component satisfies the damped harmonic oscillator equation as described for axions (Eq~\ref{eq:harmonic}), so each spatial component satisfies the same equations of motion as the scalar field previously.
...
In order to ensure the heavy photon remains non relativistic, the interaction processes that lead to thermalization must be small. These are Compton scattering $\phi e \rightarrow \phi e$ (suppressed by a factor $\chi^2$) and Compton-like scattering $\phi e \rightarrow \gamma e$. These turn out to be negligible, so that in the end, the hidden photon could also be a good cold dark matter candidate. Searches for dark matter axions can also place limits on dark matter hidden photons; in this work we use the data from our axion search to place an upper bound on the dark matter hidden photon coupling $\chi$.
...
QCD axions have mass $m_a$ and couplings $g_a$ both inversely proportional to the energy scale $f_a$, so $m_a \propto g_a$. This relation is relaxed in more general models of axion-like particles, so $m_a$ and $g_a$ are taken as independent parameters. This leaves a two-dimensional parameter space for the axion. Astrophysical observations provide stringent upper bounds on the axion mass and coupling. Cosmolgical considerations can set lower bounds on the axion mass, although these are subject to debate. Although axions arise from high energy processes, they appear as low energy observables, and there are a wide range of dedicated experimental searches looking for signatures of these axions. The majority of these experiments use the Primakoff effect, which we describe in Section \ref{sec:Prim}. The current bounds on the axion are reviewed in Section \ref{sec:CurrBounds}. Section \ref{sec:Detect} discusses the current experimental strategies, focusing on the haloscope technique.
%Astrophysical data constrains a fair portion of the possible phase space; giving an upper bound on the axion mass of approximately 1 meV and an upper bound on the axion to photon coupling of $10^{-10}$ 1/GeV. In addition cosmological constraints 
%
% Experiments that attempt to detect axions using the Primakoff effect which primarily use the axion to two photon vertex, as it is relatively straightforward to produce intense electromagnetic fields in the laboratory. Searches assuming that the ALP is the dark matter are only using microwave cavities as resonant detectors \cite{admx10} although a Rydberg atom detection experiment was performed \cite{yamamoto00}. These experiments depend on the dark matter density at Earth. There are also new ideas involving the axion as inducing an oscillating electron dipole moment in nucleons \cite{budker13}, which will be sensitive to  ALPs of a different mass than those in the microwave cavity experiments. There are other experiments looking for axion-like particles, although they do not use dark matter as the source of the WISPs. These involve looking for axions produced in the sun (CAST) \cite{cast11} or directly producing axions from the two photon vertex using a laser and strong magnetic field (ALPS-I, ALPS-II) \cite{ehret10}. The direct production or light-shining-through-wall experiments has the least number of assumptions, but these experiments (solar, microwave cavity, lsw) together form a complementary way of searching for ALPs with the two-photon vertex.
\section{Primakoff Effect}\label{sec:Prim}

Most experimental searches for axion-like particles use the Primakoff effect \cite{primakoff51}, where the axion converts to a photon in a strong magnetic field. The interaction arises from the axion to gluon coupling:
%
...
\end{figure}
\section{Current Bounds on Axion-Photon Coupling }\label{sec:CurrBounds}
%[ADD HIDDEN PHOTON BOUNDS]
This section discusses the constraints on the axion to two photon coupling $g_a{\gamma\gamma}$ and axion mass $m_a$. Astrophysical observations provide stringent upper bounds on the axion coupling and mass; if axions are the dark matter, then cosmological arguments can provide a lower bound on the axion mass, although this bound does not hold if the Peccei Quinn symmetry breaking occurs before inflation. We also discuss laboratory searches, which look for axion to photon conversion using either intense photon sources, such as the sun or lasers,  or intense axion sources, such as the galactic halo, assuming axions form the dark matter of the universe. This thesis searches for dark matter axions from the galactic halo using microwave cavities.
...
\textbf{Hidden Photons}
The astrophysical constraints on the hidden photon mass and coupling also come from energy loss arguments. The strongest limits come from observations of the solar lifetime, which constrain a large swath of parameter space in the $1\text { eV - }10^3$ eV range. See Figure~\ref{fig:hpsumplot}.
Hidden photon to photon oscillations could cause distortions in the cosmic microwave background; using the high precision observations of the CMB an upper bound can be placed on the hidden photon coupling $\chi$ of $\chi < 10^{-7}$ for masses less than 1 meV. 
\begin{figure}[htbp]
\includegraphics[width=\textwidth]{hiddenphotonexclusion}
\caption{Hidden Photon parameter space and bounds. In red are the allowed regions for hidden photon dark matter; from \cite{arias12}}.
\label{fig:hpsumplot}
\end{figure}
...
\section{Detection Strategies}\label{sec:Detect}
%For WISPs, direct searches primarily use the ALP to two photon vertex, as it is easy to produce large electromagnetic fields in the laboratory and easy to detect photons. Searches assuming that the ALP is the dark matter are only using microwave cavities as resonant detectors \cite{admx10} although a Rydberg atom detection experiment was performed \cite{yamamoto00}. These experiments depend on the dark matter density at Earth. There are also new ideas involving the axion as inducing an oscillating electron dipole moment in nucleons \cite{budker13}, which will be sensitive to  ALPs of a different mass than those in the microwave cavity experiments. There are other experiments looking for axion-like particles, although they do not use dark matter as the source of the WISPs. These involve looking for axions produced in the sun (CAST) \cite{cast11} or directly producing axions from the two photon vertex using a laser and strong magnetic field (ALPS-I, ALPS-II) \cite{ehret10}. The direct production or light-shining-through-wall experiments has the least number of assumptions, but these experiments (solar, microwave cavity, lsw) together form a complementary way of searching for ALPs with the two-photon vertex.
...
%Methods to search for dark matter axions have been ongoing for the last four decades; however, since dark matter candidates by definition interact very feebly with ordinary matter, searching for any of these particles is experimentally challenging.  
%This section provides a brief survey of the state of current experimental searches for axions. The different approaches are described with their model assumptions (e.g. axions from dark matter, or axion flux from the sun). 
%As this thesis is concerned with a dark matter axion search, I will briefly review the laboratory techniques used to search for axions. 
This section provides a brief survey of the state of current experimental searches for axions. The different approaches are described with their model assumptions (e.g. axions from dark matter, or high axion flux from the sun). The main experimental strategies use the Primakoff effect. They are: solar helioscopes, light-shining-through-wall (photon regeneration) experiments, and microwave cavity haloscopes. In addition there are non-Primakoff searches, looking for macroscopic forces or oscillating electric dipole moments.
...
It is also possible for axions to mediate short range forces. Experiments have been made testing this hypothesis \cite{tullney13}, \cite{hoedl11} and find no evidence for pseudoscalar forces as predicted by an axion. This experiment places upper bounds on the product of two coupling factors for a certain length range; it is not straightforward to compare this to the Primakoff effect experiments, but it seems that they also have no evidence.
...
The axion form factor is only non-zero when the scalar product of the cavity electric field and the external magnetic field is non-zero. This means, for a non-zero form factor, we must operate with transverse magnetic modes which have an axial electric field. The $\text{TM}_{0n0}$ modes have a non-zero form factor and $C$ scales as 
\begin{align}
C_{0n0} \propto \frac{4}{x_{0n}^2}
\end{align}
for cylindrical cavities, where $x_{0n}$ is the nth root of the first order Bessel function $J_0(r)$. This goes down approximately as $n^2$ so with increasing mode index, so the form factor goes down quickly for higher order modes.
...
The tuning rod consists of a G-10 rod, 0.125" diameter. The waveguides are made of several pieces - at the top of the insert there is a six inch section of stainless steel copper coated waveguide, and the rest is copper waveguide. This is to reduce the heat load on the bottom of the cryostat. The waveguides used throughout are WR28 - which means rectangular waveguide with a lower cutoff frequency of 21.5 GHz.
...
We couple inductively by picking up overlap in the magnetic field of the waveguide mode $\text{TE}_{10}$ and the azimuthal field of the cavity  $\text{T}\text{M}_{020}$ mode. As discussed in the previous chapter, operating in the $\text{T}\text{M}_{020}$ mode decreases the form factor C from the maximal value, but allows us to have a larger volume, which increases the sensitivity. It is also easier to machine and work with a cavity that is larger than the waveguides. The  $\text{T}\text{M}_{0n0}$modes have only non-zero electric field in the z direction 
...
The $\text{T}\text{M}_{020}$ mode must have good connection between the walls and the caps because the alternating E field means that there are vertical currents that switch direction and flow in the vertical axis so if they cannot flow to the top or bottom cap, the mode's Q will be severely degraded.
%\subsubsection{HFSS}
...
The height of the cavity was set to be $L=0.34$ in to avoid a mode crossing with the higher  $\text{T}\text{E}_{112}$ mode that appeared in the simulation. However, when the cavity was built the modes were separated by at least 2 GHz and there is no danger of a mode crossing over the range in which we scan. The other close mode is the  $\text{T}\text{M}_{010}$ mode, which is at 17 GHz.
...
The cavity was simulated in Ansoft HFSS. Surface finish plays a large role in the Q of the cavity - using a surface roughness tester we determined that the surface roughness of the cavity surface was less than 2 $\mu$in. This value leads to negligible effects in the simulations and was therefore neglected.  
...
\begin{align}
\delta = 2.8 \times 10^{-5}\text{ cm} (\frac{\text{GHz}}{f})^{1/3}
\end{align}

For the $\text{T}\text{M}_{020}$ modes the theoretical unloaded Q is $$Q_0 = \frac{L}{L+R}\frac{R}{\delta}$$

in general a cylindrical $\text{T}\text{M}_{mnp}$ mode has Q equal to \cite{poole96}
\begin{align}
Q_0 = \frac{\lambda}{\delta}\frac{[x_{mn}^2 + (p\pi a/d)^2]^{1/2}}{2\pi(1+2a/d)}
\end{align}

 where $a$ is the radius and $d$ the length. Then for the $\text{T}\text{M}_{020}$ mode, this simplifies to 
 \begin{align}
...
\caption{Room tempŽrature cavity frequency, Q, and coupling. Plot of reflection coefficient $|\Gamma|^2$ vs detuning from resonance $f_{\text{res}}$. Unloaded Q and coupling coefficient calculated via formula in Ref \cite{kajfez94}}
\end{subfigure}
\begin{subfigure}{\textwidth}
\centering
...
for $T_1 = 15 K$ and $G_1 = 27$ dB, for the second amplifier with noise temperature given by the vendor of $100$ K, this would only contribute $100/10^{2.7} = 0.1$ K to the total noise temperature.
...
\begin{align}
T_{sys} = \text{max}(T_{HEMT}, T_{load}) + T_N
\end{align}
...
The data acquisition system (computer plus sensors plus instruments) measures important parameters, e.g. temperatures, pressures. As discussed earlier, the important cavity parameters are the cavity quality factor Q, center frequency of the cavity, strength of magnetic field, and cavity physical temperature. Once these quantities are measured, we manually adjust the tuning rod to shift the cavity resonance by 3 MHz. Once the cavity has been tuned, we use the network analyzer to record the cavity frequency, Q, and bandwidth, and then a 1 hr time trace is acquired as a two channel system and saved to disk in little-endian binary format as signed 8-bit integers.
...
\caption{Histogram of power fluctuations}
\end{figure}
...
\chapter{Noise Temperature}
Let's start with a simple model: a noiseless amplifier with two traveling waves $$a_{n}$$ and $$b_{n}$$ with a source impedance $$\Gamma_{s}$$. If you were to do a scattering measurement, you would get this relationship:

$$\begin{bmatrix} b_1 - b_n\\b_2\end{bmatrix} = \begin{bmatrix}s_{11} & s_{12} \\s_{21} & s_{22} \end{bmatrix} \times \begin{bmatrix}a_1 + a_n  \\a_2\end{bmatrix}$$, where $a_i$ is the ingoing wave at port i and $b_i$ is the outgoing wave at port i.

Now, for some assumptions. 

1. Assume that you can make $a_n$ and $b_n$ uncorrelated:

$$ \overline{a_nb_n^*} = 0 $$

This happens to be true when the characteristic impedance Z is equal to the optimal source impedance (see Penfield's paper for the exact expression).

The strength of $a_n$ defines a temperature $T_a$:

$$T_a = \frac{|a_n|^2}{k\Delta f}$$

and likewise with$T_b$.

The minimum noise figure when be $$F-1 = \frac{T_a}{T_0}$$.

If the source impedance is $$Z_s = R_s + jX_s$$, the reflection coefficient of the source is $$\Gamma_s = \frac{Z_s-Z}{Z_s+Z^*}$$

Then the noise figure is $$F-1 = \frac{T_a+T_b|\Gamma_s|^2}{T_0(1-|\Gamma_s|^2)} =\frac{T_a}{T_0} + \frac{T_a+T_b}{T_0}\frac{{|\Gamma_s}|^2}{(1-|\Gamma_s|^2)}$$

For a reflection coefficient at the amplifier of $\Gamma_a$, the total incident noise wave becomes $$\frac{1}{1-\Gamma_a\Gamma_s}T_0$$

If a lossless line of length L is introduced between the souce and amplifier, we must use $$\Gamma_s(z=L) = \Gamma_s e^{-2j\beta L}$$ where $$\lambda = \frac{2\pi}{\beta}$$ and $$\beta = \omega \sqrt{LC}$$
...
\caption{Resonance severely suppressed. Taken Feb 05, 2014.}
\end{subfigure}
\end{figure}
